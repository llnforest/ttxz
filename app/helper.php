...
if(!function_exists('operateResult')){
     ...
     * @return array
     */
    function operateResult($default,$operate = 'add'){
        if ($default) {
...
if(!function_exists('priceSplit')){
    /**
     * 价格区间分隔
     * @param boolean $price_string
     * @param string $split
     * @return array
     */
    function priceSplit($price_string,$split = '-'){
        if(mb_substr($price_string,-2) == '以内'){
            return ['lt',str_replace('以内','',$price_string)];
        }elseif(mb_substr($price_string,-2) == '以上'){
            return ['gt',str_replace('以上','',$price_string)];
        }else{
            return ['between',str_replace('-',',',$price_string)];
        }
    }
}
